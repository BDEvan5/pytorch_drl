...
class PPO:
    ...
    def train(self, next_state=None):
        ...
        for i in range(K_epoch):
            ...
            log_ratio = torch.log(probs_for_actions) - torch.log(prob_a)  # a/b == exp(log(a)-log(b))
            ratio = torch.exp(log_ratio)  # a/b == exp(log(a)-log(b))

            # implementing the clipped surrogate objective
            actor_loss = ratio * advantage - log_ratio**2
            critic_loss = F.smooth_l1_loss(self.critic.v(states) , td_target.detach())
