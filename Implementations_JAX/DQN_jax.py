...
class QNetworkDQN(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        x = nn.Dense(NN_LAYER_1)(x)
        x = nn.relu(x)
        x = nn.Dense(NN_LAYER_2)(x)
        x = nn.relu(x)
        x = nn.Dense(1)(x)
...
class OffPolicyBuffer:
    def __init__(self, state_dim, action_dim):     
        ...
        self.states = jnp.empty((MEMORY_SIZE, state_dim))
        self.actions = jnp.empty((MEMORY_SIZE, action_dim))
        self.next_states = jnp.empty((MEMORY_SIZE, state_dim))
        self.rewards = jnp.empty((MEMORY_SIZE, 1))
        self.dones = jnp.empty((MEMORY_SIZE, 1))
    ...
    def sample(self, batch_size):
        ind = jnp.random.randint(0, self.ptr-1, size=batch_size)
        states = jnp.empty((batch_size, self.state_dim))
        actions = jnp.empty((batch_size, self.action_dim))
        next_states = jnp.empty((batch_size, self.state_dim))
        rewards = jnp.empty((batch_size, 1))
        dones = jnp.empty((batch_size, 1))
...
class DQN:
    def __init__(self, obs_space, action_space):
        ...
        self.model_params = self.model.init(key, np.ones(obs_space, 1))
    ...
    def act(self, state):
        if jax.random.random() < self.exploration_rate:
            return jax.random.randint(0, self.action_space-1)
        else: 
